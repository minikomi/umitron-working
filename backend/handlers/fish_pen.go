...
func decodeFishPenRequest(r *http.Request) (*FishPenRequest, error) {
	...
	fpr.MakerModelName = trimOptionalString(fpr.MakerModelName)
	fpr.Description = trimOptionalString(fpr.Description)
	fpr.Material = trimOptionalString(fpr.Material)
	fpr.NetMaterial = trimOptionalString(fpr.NetMaterial)
...
func (h *FishPenHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	fp, err = h.fishPenService.Create(farmID, fp)
	if err != nil {
		if errors.Is(err, types.ErrDuplicate) {
			respondJSON(w, http.StatusConflict, map[string]string{"error": "FishPen already exists"})
			return
		}
